...
# you must set is_decoder as True for generation
config.is_decoder = True
encoder = RobertaModel.from_pretrained(
    "microsoft/unixcoder-base", config=config)

model = Seq2Seq(encoder=encoder, decoder=encoder, config=config,
                beam_size=3, max_length=150,
                sos_id=tokenizer.convert_tokens_to_ids(["<mask0>"])[0],
                eos_id=tokenizer.sep_token_id).eval()

model.to(device)
checkpoint_prefix = 'generate/gen_model.bin'
model.load_state_dict(torch.load(checkpoint_prefix, map_location=device))


def code_gen_inference(nl, model, tokenizer):
    # NL length
    max_source_length = 350
    # tokenization
    source_tokens = tokenizer.tokenize(nl)[:max_source_length-5]
    source_tokens = [tokenizer.cls_token, "<encoder-decoder>",
                     tokenizer.sep_token]+source_tokens+["<mask0>", tokenizer.sep_token]
    ...
    # inference
    preds = model(source_ids).squeeze(0)
    codes = []
    for pred in preds:
        tokens = list(pred.cpu().detach().numpy())
        if 0 in tokens:
